...
# 14 重定向 与 错误处理
# 14.1  redirect() 函数把用户重定向到其它地方。
@app.route('/error1')
def error1():
	return redirect(url_for('template_hello'))

# 14.2 放弃请求并返回错误代码，用 abort() 函数
@app.route('/error2')
def error2():
	abort(404)
...
# 15 session 会话  实现登录与登出
@app.route('/session')
def index_session():
	if 'username' in session:
		print(escape(session['username']))
		return '现在登录的是： {}'.format(escape(session['username']))
